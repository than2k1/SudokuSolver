...
class MyGame(object):
    ...
    # Return empty spot on grid with least amount of options
    def get_next(self, boardSize, state):
        min_option_len = boardSize
        # row = 0
            ...
            for col in range(boardSize):
                if state[row][col] == 0:
                    if state[row][col] == 0:
                        ...
                        options = self.filter_square(options, state, row, col)
                        if len(options) < min_option_len:
                            min_option_len = len(options)
                            options = []
                            nextRow = row
                            nextCol = col
